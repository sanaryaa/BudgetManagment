...
<body class="d-flex flex-column flex-lg-row">
  ...
  <section class="body-content">
    <div class="container mt-5 ">
      <!-- style borrow button to atcive  -->
      <?php $activeDebt='borrow';include './includes/debtFields.php'; ?>
      <div class="items d-flex flex-column justify-content-center align-items-center w-100">
        <?php
        // display  borrow items in card that belonging to the user
        $user_id = $_SESSION['user_id'];
        $select_query = "SELECT * FROM borrow_items WHERE user_id = {$_SESSION['user_id']} and month(date)={$_SESSION['month']} and year(date)={$_SESSION['year']}";
        ...
        if (mysqli_num_rows($result_query) > 0) {
          while ( $row = mysqli_fetch_assoc($result_query)) {
              ...
              // when data-bs-target="#'. $borrow_id .'" is clicked, it will open a modal with an id attribute equal to the value of the $borrow_id variable.
              echo '
      ...
      <a href="#" data-bs-toggle="modal" data-bs-target="#' . $borrow_id . '"><ion-icon name="pencil-sharp"></ion-icon></a>
         <a href="deleteItems.php?delete_borrow_id=' . $borrow_id . '"><ion-icon name="trash-outline" ></ion-icon></a>
      </div>
    </div>



    <div class="modal" id="' . $borrow_id . '" aria-labelledby="exampleModalLabel">
      <div class="modal-dialog modal-dialog-centered">
        ...
        <input type="submit" class="btnModal btn-add" name="updateborrow" value="Save Change">
         </div>
            </form>
          </div>
            ...
            <div class="modal-body px-5">
              ...
              if (isset($_POST["submit"])) {
                ...
                $user_id = $_SESSION['user_id'];
                  ...
                  $insert_query = "INSERT INTO borrow_items (person_name,amount,date,user_id) VALUES ( '$person_name', '$amount','$date','$user_id')";
