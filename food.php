...
<body class="d-flex flex-column flex-lg-row">
  ...
  <section class="body-content">
    <div class="container mt-5 mt-lg-0">
      <!-- style food button to atcive  -->
      <?php $activeExpen = 'food';
      include './includes/expenses.php'; ?>
      <div class="items d-flex flex-column justify-content-center align-items-center w-100">
        <?php
        // display  food items in card that belonging to the user
        $user_id = $_SESSION['user_id'];
        $select_query = "SELECT * FROM food_items WHERE user_id = {$_SESSION['user_id']} and month(date)={$_SESSION['month']} and year(date)={$_SESSION['year']}";
        ...
        if (mysqli_num_rows($result_query) > 0) {
          while ( $row = mysqli_fetch_assoc($result_query)) {
              ...
              // when data-bs-target="#'. $food_id .'" is clicked, it will open a modal with an id attribute equal to the value of the $food_id variable.
              echo '
      <div class="cards d-flex align-items-center w-100" style="border-left: 4px solid #f59a51;">
      <div class="card-body ps-4">
        <h5 class="card-title"> ' . $item_name . '</h5>
        <p class="date pt-2 mb-2 ">' . $date . '</p>
      </div>
      <div class="price me-2 d-flex justify-content-center align-items-center" style="background-color:#fdefe2;">
        <h4>$' . $price . '</h4>
      </div>
      <div class="cardIcons  pe-sm-4" >
      <a href="#" data-bs-toggle="modal" data-bs-target="#' . $food_id . '"><ion-icon name="pencil-sharp"></ion-icon></a>
         <a href="deleteItems.php?delete_food_id=' . $food_id . '"><ion-icon name="trash-outline" ></ion-icon></a>
      </div>
    </div>



    <div class="modal" id="' . $food_id . '" aria-labelledby="exampleModalLabel">
      <div class="modal-dialog modal-dialog-centered">
        ...
        <input type="submit" class="btnModal btn-add" name="updateFood" value="Save Change">
         </div>
            </form>
          </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body px-5">
              ...
              if (isset($_POST["submit"])) {
                ...
                $user_id = $_SESSION['user_id'];
                $errors = array();
                  $insert_query = "INSERT INTO food_items (item_name,price,date,user_id) VALUES ( '$item_name', '$price','$date','$user_id')";
